...
| **Phase 4: Streamlit ページ統合** | 顧客向け査定 UI（類似物件 + 区間の表示） | 🔄 進行中（配信設定のみ残） |
...
- ✅ 物件選択 UI（サイドバーで 行政区 → 種類 → 物件 を選択）
- ✅ 予測値カード（点予測・モデル予測区間・乖離 ratio・類似件数）
- ✅ 類似物件テーブル（上位5件）
- ✅ 類似物件の実価格分布ヒストグラム
...
- 取引データは `features.csv`（`取引年` を含む）を直接読み込む方式とし、当初想定の
  「`test_predictions.csv` への `取引年` 追加」は不要になった。
- 信頼度判定 `assess_reliability()` は `band`（軽度/中度/重度）・`has_overlap`・
  `n_used` を総合: 低=重度 or 区間非重複、中=中度 or 類似物件 < 10 件、高=それ以外。

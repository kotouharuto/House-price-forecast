...
flowchart TD
    ...
    subgraph P0["✅ P0: 基盤"]
        direction TB
        P0A["CSV読込\n(BI_PREDICTIONS_PATH で切替可)"]
        P0B["@st.cache_data\nキャッシュ化"]
        P0C["集計モジュール\naggregate / master / format"]
        P0D["ユニットテスト\n(test_aggregate / master / format)"]
        P0A --> P0B --> P0C --> P0D
    end

    P0 --> P1

    subgraph P1["⚠️ P1: BIグラフ (機能完了 / 一部UX未達)"]
        direction TB
        P1A["KPIサマリ\n件数・R²・MAE・RMSE・MAPE・Median APE"]
        P1B["Plotlyグラフ群\n散布図・誤差分布・価格帯別・\n行政区/駅ランキング・特徴量関係"]
        P1C["サイドバーフィルタ\n行政区・駅・種類・価格帯・\n面積・築年数・山手線内側"]
        P1D{{"⚠️ 残課題\n金額表示・ツールチップ\nサンプリング明示・色スケール"}}
        P1A & P1B & P1C --> P1D
    ...
    style P0 fill:#dcfce7,stroke:#22c55e
